...
new class extends Component {
    ...
    public function store()
    {
        $this->validate([
            ...
            'nama.unique' => 'Nama brand ini sudah terdaftar.',
        ]);
        ...
        $this->closeModal();
        session()->flash('success', 'Brand baru berhasil ditambahkan.');
    }
    ...
    public function edit($id)
    {
        ...
        $this->keterangan = $brand->keterangan;
    ...
    {
        $this->validate([
            'nama' => 'required|unique:brands,nama,' . $this->selectedId,
            'keterangan' => 'nullable|string|max:255',
        ]);

        Brand::findOrFail($this->selectedId)->update([
            'nama' => $this->nama,
            'keterangan' => $this->keterangan,
        ]);

        $this->closeModal();
        session()->flash('success', 'Data brand berhasil diperbarui.');
    }
    ...
    {
        return [
            'items' => Brand::where('nama', 'like', "%{$this->search}%")
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 border-b border-slate-100 text-slate-500 font-bold uppercase text-xs">
                    <tr>
                        <th class="px-6 py-4">Nama Merk / Brand</th>
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-3">
                                <div class="flex justify-center gap-2">
                                    ...
                                    <button wire:click="delete({{ $item->id }})" wire:confirm="Yakin ingin menghapus brand '{{ $item->nama }}'?" class="p-1.5 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
                    ...
                    @empty
                        <tr><td colspan="3" class="px-6 py-8 text-center text-slate-400">Belum ada data brand.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 class="font-bold text-lg text-slate-800">{{ $isEditMode ? 'Edit Brand' : 'Tambah Brand' }}</h3>
            ...
            <div class="p-6">
                <form wire:submit="{{ $isEditMode ? 'update' : 'store' }}" class="space-y-4">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1">Nama Merk / Brand <span class="text-rose-500">*</span></label>
                        <input wire:model="nama" type="text" class="w-full rounded-xl border-slate-200 text-sm focus:ring-blue-500" placeholder="Contoh: Dell, Samsung, IKEA">
                    ...
                    <div>
                        ...
                        <textarea wire:model="keterangan" rows="3" class="w-full rounded-xl border-slate-200 text-sm focus:ring-blue-500" placeholder="Deskripsi pabrikan..."></textarea>
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-4 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-100 transition-all">
                            {{ $isEditMode ? 'Simpan Perubahan' : 'Simpan Brand' }}
                        </button>
                    </div>
                </form>
            </div>
